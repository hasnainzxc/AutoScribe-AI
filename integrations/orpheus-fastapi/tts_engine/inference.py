...
import wave
import numpy as np
"""Inference utilities for Orpheus TTS with lazy heavy imports.

This update defers importing torch (ROCm/CUDA) and sounddevice (PortAudio)
until actually needed, so FastAPI can start instantly and reliably.
"""
import argparse
import threading
import queue
...
# Lazy hardware detection to avoid heavy imports at startup
try:
    import psutil as _psutil
except Exception:
    _psutil = None

HIGH_END_GPU = False  # Will be set on first detection
_CUDA_AVAILABLE: Optional[bool] = None  # True if any GPU (CUDA/ROCm) is available
_GPU_INFO_PRINTED = False
_GPU_INFO: Optional[Dict[str, Any]] = None

def _detect_amd_rocm() -> Optional[Dict[str, Any]]:
    """Try to detect AMD GPU via ROCm tools (rocminfo/rocm-smi). Returns info dict if found."""
    # Try rocminfo
    try:
        res = subprocess.run(["rocminfo"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, timeout=3)
        out = res.stdout or ""
        # Look for GPU name (e.g., AMD Radeon RX 6800M) and gfx arch (e.g., gfx1030)
        name_match = None
        gfx_match = None
        for line in out.splitlines():
            ...
            if not gfx_match:
                ...
                if m:
                    gfx_match = m.group(0)
            if name_match and gfx_match:
                break
        if name_match or gfx_match:
            return {"backend": "ROCm", "name": name_match or "AMD GPU", "arch": gfx_match}
    except Exception:
        pass
    # Try rocm-smi for name and VRAM
    try:
        res = subprocess.run(["rocm-smi", "--showproductname", "--showmeminfo", "vram"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, timeout=3)
        out = res.stdout or ""
        name = None
        vram_gb = None
        for line in out.splitlines():
            if name is None and ("Product Name" in line or "Card series" in line):
                parts = line.split(":", 1)
                if len(parts) == 2:
                    name = parts[1].strip()
            if vram_gb is None and ("Total Memory" in line or "VRAM Total" in line):
                # Extract number in MiB
                ...
                if m:
                    try:
                        ...
                        pass
        if name or vram_gb:
            return {"backend": "ROCm", "name": name or "AMD GPU", "vram_gb": vram_gb}
    except Exception:
        pass
    return None

def _detect_gpu_class() -> tuple[bool, bool]:
    """Lazily detect GPU availability and whether it's a high-end device.

    Works for CUDA and ROCm builds of PyTorch, or via ROCm tools. Returns (gpu_available, high_end_gpu).
    """
    global _CUDA_AVAILABLE, HIGH_END_GPU, _GPU_INFO
    if _CUDA_AVAILABLE is not None:
        return _CUDA_AVAILABLE, HIGH_END_GPU
    # First try PyTorch CUDA/ROCm
    try:
        import torch as _torch  # Lazy import
        cuda = _torch.cuda.is_available()
        _CUDA_AVAILABLE = bool(cuda)
        if cuda:
            try:
                props = _torch.cuda.get_device_properties(0)
                gpu_mem_gb = float(getattr(props, 'total_memory', 0)) / (1024**3)
                name = getattr(props, 'name', 'GPU')
                backend = 'ROCm' if getattr(_torch.version, 'hip', None) else 'CUDA'
                major = getattr(props, 'major', None)
                minor = getattr(props, 'minor', None)
                _GPU_INFO = {"backend": backend, "name": name, "vram_gb": gpu_mem_gb, "cc_major": major, "cc_minor": minor}
                HIGH_END_GPU = gpu_mem_gb >= 16.0 or (gpu_mem_gb >= 12.0 and (major is None or major >= 7))
                return _CUDA_AVAILABLE, HIGH_END_GPU
            except Exception:
                pass
    except Exception:
        pass
    # Fallback: try ROCm tool detection
    amd = _detect_amd_rocm()
    if amd:
        _GPU_INFO = amd
        vram_gb = amd.get("vram_gb")
        HIGH_END_GPU = bool(vram_gb and (vram_gb >= 16.0 or vram_gb >= 12.0))
        _CUDA_AVAILABLE = True  # Signal that a GPU is present
    else:
...
def generate_tokens_from_api(prompt: str, voice: str = DEFAULT_VOICE, temperature: float = TEMPERATURE, 
    ...
    cuda_available, high_end = _detect_gpu_class()
    if high_end:
...
def stream_audio(audio_buffer):
    ...
    try:
        # Lazy import to avoid PortAudio and device probing at startup
...
def generate_speech_from_api(prompt, voice=DEFAULT_VOICE, output_file=None, temperature=TEMPERATURE, 
    ...
    if not _GPU_INFO_PRINTED and not IS_RELOADER:
        try:
            if cuda_available:
                if _GPU_INFO:
                    name = _GPU_INFO.get("name", "GPU")
                    backend = _GPU_INFO.get("backend", "GPU")
                    vram_gb = _GPU_INFO.get("vram_gb")
                    cc_major = _GPU_INFO.get("cc_major")
                    cc_minor = _GPU_INFO.get("cc_minor")
                    extra = ""
                    if backend == 'CUDA' and cc_major is not None and cc_minor is not None:
                        extra = f", compute capability {cc_major}.{cc_minor}"
                    elif backend == 'ROCm' and _GPU_INFO.get("arch"):
                        extra = f", arch {_GPU_INFO['arch']}"
                    vram_str = f" — {vram_gb:.1f} GB VRAM" if isinstance(vram_gb, (int, float)) else ""
                    print(f"🖥️ Hardware: {name} ({backend}){vram_str}{extra}" + (" (high-end)" if high_end else ""))
                else:
                    print("🖥️ Hardware: GPU detected" + (" (high-end)" if high_end else ""))
                print("🚀 Using high-performance optimizations" if high_end else "🚀 Using GPU-optimized settings")
            else:
                if _psutil is not None:
                    cpu_cores = _psutil.cpu_count(logical=False)
                    cpu_threads = _psutil.cpu_count(logical=True)
                    ram_gb = _psutil.virtual_memory().total / (1024**3)
                    print("🖥️ Hardware: CPU only (No GPU detected)")
    ...
    print(f"Using voice: {voice}, GPU acceleration: {'Yes (High-end)' if high_end else 'Yes' if cuda_available else 'No'}")
